...
/**
 * JQuery Rest Client Plugin v1.4.0-pre https://git.oschina.net/anylain/RestClient
 *
...
(function ($) {
    ...
    $.RestClient = function (options) {
        ...
        this._compatibleHandler = function (method, request) {
            ...
            switch (request.compatible) {
                ...
                default:
                    throw new Error('Unsupport rest compatible mode "' + request.compatible + '"');
            }
        };
        ...
        // 构造请求地址
        this.buildUrl = function (sourceUrl, urlParams, pathParams, queryParams, ext) {
            ...
            var result = !sourceUrl ? '' : _gsub(sourceUrl, /\{(.+?)\}/,
                function (match) {
                    ...
                    if (value === undefined || value === null) {
                        $.error('Can\'t found "' + key + '" from pathParams or urlParams.');
                    }
            ...
            if (urlParams) {
                $.each(urlParams, function (key, value) {
                    if ($.inArray(key, usedPathParams) === -1 && queryParams[key] === undefined) {
            ...
            result.success = function (data, textStatus, jqXHR) {
                if (result.deserializeResponseBody) {
                    try {
                        ...
                        $
                            .error('Deserialize response body fault. ' + e.message);
                    }
                }
                if (successHandler) {
        ...
        this.sendRequest = function (method, request, addon) {
            ...
            try {
                ...
                var tryCallComplete = function () {
                    var completeHandler = (addon && addon.complete) || (request && request.complete) || _this.getOptions().complete;
                    if (completeHandler) {
                        completeHandler();
                    }
                };
                var errorHandler = (addon && addon.error) || (request && request.error) || _this.getOptions().error;
                if (errorHandler) {
    ...
    $.RestClient.options.json = {
        ...
        // 默认的错误处理 可接受的参数有 message, jqXHR, textStatus, errorThrown
