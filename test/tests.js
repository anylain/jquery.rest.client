...
QUnit.test("URL Build Test", function (assert) {
    var rc = new $.RestClient();
    var url, good;
    url = rc.buildUrl('users/{id}', {id: 123});
    good = 'users/123';
    assert.equal(url, good, "Simple url params");

    url = rc.buildUrl('users/{id}', null, {id: 123});
    good = 'users/123';
    assert.equal(url, good, "Simple path params");

    url = rc.buildUrl('users', null, null, {id: 123});
    good = 'users?id=123';
    assert.equal(url, good, "Simple query params");

    url = rc.buildUrl('users/{id}', {id: 456}, {id: 123});
    good = 'users/123';
    assert.equal(url, good, "Path params priority than the url params");

    url = rc.buildUrl('users', {id: 456}, null, {id: 123});
    good = 'users?id=123';
    assert.equal(url, good, "Query params priority than the url params");

    url = rc.buildUrl('users/{id}/book/{book}', {id: 123, key: "python", key1: "lulu"}, {book: "rest_client"}, {key: "javascript"}, '.pdf');
    good = 'users/123/book/rest_client.pdf?key=javascript&key1=lulu';
    ...
    url = rc.buildUrl('users/{id}/books/{book}', {id: 123, book: 'Spring实战(第三版)', page: 12, keyword: 'http://spring.io/'}, null, null, '.json');
    good = 'users/123/books/Spring%E5%AE%9E%E6%88%98(%E7%AC%AC%E4%B8%89%E7%89%88).json?page=12&keyword=http%3A%2F%2Fspring.io%2F';
